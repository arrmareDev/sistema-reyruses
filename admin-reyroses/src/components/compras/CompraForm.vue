<template>
  <Teleport to="body">
    ...
    >
      <div :class="['rounded-2xl p-6 md:p-8 w-full max-w-3xl shadow-2xl max-h-[92vh] overflow-y-auto', isDarkTheme ? 'bg-[#16161a] border border-[#2a2a32]' : 'bg-white border border-gray-200']">
        <div :class="['flex items-center justify-between mb-6 pb-4 border-b', isDarkTheme ? 'border-[#2a2a32]' : 'border-gray-200']">
          ...
          <button @click="close" :class="['p-1', isDarkTheme ? 'text-zinc-500 hover:text-white' : 'text-gray-400 hover:text-gray-900']">
        ...
        <form @submit.prevent="handleSubmit" class="space-y-6">
          ...
          <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <div>
              ...
              <input v-model="form.tipo_cambio" type="number" step="0.0001" min="0" placeholder="3.75" required :class="inputClass" />
            </div>
            <div>
              <label :class="labelClass">Comisión Western</label>
              <input v-model="form.comision_western" type="number" step="0.01" min="0" placeholder="Aún no confirmada" :class="inputClass" />
            </div>
            <div>
            ...
            <div>
              ...
              <input v-model="form.saldo_inversion_usado" type="number" step="0.01" min="0" :max="inversionStore.saldoActual" :class="inputClass" />
          ...
          <div>
            <div class="flex items-center justify-between mb-3">
              <h3 :class="['text-xs font-semibold uppercase tracking-widest', isDarkTheme ? 'text-zinc-500' : 'text-gray-500']">
            ...
            <div class="space-y-4">
              <div
                v-for="(tabaco, index) in form.tabacos"
                :key="index"
                :class="['p-4 rounded-xl border', isDarkTheme ? 'bg-[#1e1e24] border-[#2a2a32]' : 'bg-gray-50 border-gray-200']"
              >
                <div class="flex items-center justify-between mb-3">
                  <span :class="['text-xs font-bold uppercase', isDarkTheme ? 'text-zinc-400' : 'text-gray-500']">Tabaco #{{ index + 1 }}</span>
                  <button
                    v-if="form.tabacos.length > 1"
                    type="button"
                    @click="removeTabaco(index)"
                ...
                <div class="grid grid-cols-2 gap-3 mb-4">
                  ...
                  <div>
                    ...
                    <input v-model="tabaco.costo_tabaco_usd" type="number" step="0.01" min="0" required :class="inputClass" />
                  </div>
                </div>
                ...
                <div class="grid grid-cols-5 gap-2">
                  <div v-for="tallo in TALLOS" :key="tallo" class="text-center">
                    <span :class="['text-[10px] font-bold block mb-1', isDarkTheme ? 'text-zinc-600' : 'text-gray-400']">{{ tallo }}cm</span>
                    <input
                      v-model="tabaco.desglose[tallo]"
                      type="number"
                      min="0"
                ...
                <p :class="['text-xs font-semibold mt-2', totalPaquetes(tabaco) === 14 ? 'text-emerald-500' : 'text-amber-500']">
          ...
          <div :class="['p-4 rounded-xl border space-y-2', isDarkTheme ? 'bg-[#1e1e24] border-[#2a2a32]' : 'bg-emerald-50 border-emerald-100']">
            <h3 :class="['text-xs font-semibold uppercase tracking-widest mb-2', isDarkTheme ? 'text-zinc-500' : 'text-gray-500']">
              Proyección
            </h3>
            <ResumenRow label="Subtotal tabacos (USD)" :value="'US$ ' + resumen.subtotalUsd.toFixed(2)" :dark="isDarkTheme" />
            <ResumenRow label="Subtotal en soles" :value="'S/ ' + resumen.subtotalPen.toFixed(2)" :dark="isDarkTheme" />
            <ResumenRow label="Comisión tarjeta (5%)" :value="'S/ ' + resumen.comisionTarjeta.toFixed(2)" :dark="isDarkTheme" />
            <ResumenRow label="Costo total" :value="'S/ ' + resumen.costoTotal.toFixed(2)" :dark="isDarkTheme" bold />
            <ResumenRow label="Ingreso proyectado (venta)" :value="'S/ ' + resumen.ingresoProyectado.toFixed(2)" :dark="isDarkTheme" />
            <ResumenRow
              label="Ganancia estimada"
              :value="'S/ ' + resumen.gananciaEstimada.toFixed(2)"
              :dark="isDarkTheme"
              bold
