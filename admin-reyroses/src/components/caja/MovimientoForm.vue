        ...
        <form @submit.prevent="handleSubmit" class="space-y-4">
          <div class="grid grid-cols-2 gap-2">
            <button type="button" @click="form.tipo = 'ingreso'"
              :class="['py-2.5 rounded-xl text-sm font-semibold transition-all flex items-center justify-center gap-2', form.tipo === 'ingreso' ? 'bg-pine-500 text-white' : isDarkTheme ? 'bg-ink-800 text-ink-500' : 'bg-gray-100 text-gray-500']">
              <ArrowDownCircle class="w-4 h-4" :stroke-width="2" />
              Ingreso
            </button>
            <button type="button" @click="form.tipo = 'egreso'"
              :class="['py-2.5 rounded-xl text-sm font-semibold transition-all flex items-center justify-center gap-2', form.tipo === 'egreso' ? 'bg-brick-500 text-white' : isDarkTheme ? 'bg-ink-800 text-ink-500' : 'bg-gray-100 text-gray-500']">
          ...
          <div>
            <label class="block text-[11px] font-semibold uppercase tracking-wide mb-1.5 text-ink-600">Concepto</label>
