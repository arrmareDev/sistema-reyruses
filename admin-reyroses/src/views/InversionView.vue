          ...
          <tbody :class="['text-sm', isDarkTheme ? 'text-zinc-300' : 'text-gray-700']">
            <tr v-for="mov in inversionStore.movimientos" :key="mov.id"
              ...
              <td class="p-4 text-center">
                <span
                  :class="['px-2.5 py-1 rounded-full text-[10px] font-bold uppercase', mov.tipo === 'deposito' ? 'bg-emerald-500/20 text-emerald-400' : 'bg-amber-500/20 text-amber-400']">
                  {{ mov.tipo }}
                </span>
              </td>
              <td
                :class="['p-4 text-right font-bold font-mono', mov.tipo === 'deposito' ? 'text-emerald-500' : 'text-amber-500']">
                {{ mov.tipo === 'deposito' ? '+' : '-' }} S/ {{ Number(mov.monto).toFixed(2) }}
              </td>
            </tr>
            <tr v-if="inversionStore.movimientos.length === 0">
              <td colspan="4" :class="['p-10 text-center', isDarkTheme ? 'text-zinc-500' : 'text-gray-500']">
      ...
      <div v-if="showForm"
        ...
        <div
          ...
          <form @submit.prevent="handleSubmit" class="space-y-4">
            ...
            <div>
              <label
                class="block text-[11px] font-semibold uppercase tracking-wide mb-1.5 text-zinc-500">Descripción</label>
